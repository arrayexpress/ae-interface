        ...
        <xsl:if test="position() >= $pFrom and not(position() > $pTo)">
            <tr id="{$vExpId}_main" class="{$vDetailedViewMainTrClass}">
                ...
                <td class="td_main_img ae_align_center {$vDetailedViewMainTdClass}">
                    <div>
                        <xsl:choose>
                            <xsl:when test="@loadedinatlas"><a href="${interface.application.link.atlas.exp_query.url}{$vAccession}?ref=aebrowse"><img src="{$context-path}/assets/images/basic_tick.gif" width="16" height="16" alt="*"/></a></xsl:when>
