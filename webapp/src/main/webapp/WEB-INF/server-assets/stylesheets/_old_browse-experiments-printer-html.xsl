...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
        ...
        <html lang="en">
            <xsl:call-template name="page-header-plain">
                ...
                <xsl:with-param name="pExtraCode">
                    <link rel="stylesheet" href="{$context-path}/assets/stylesheets/ae_common_20.css" type="text/css"/>
                    <link rel="stylesheet" href="{$context-path}/assets/stylesheets/ae_browse_printer_20.css" type="text/css"/>
                    ...
                    <script src="{$context-path}/assets/scripts/ae_browse_printer_20.js" type="text/javascript"/>
                </xsl:with-param>
            </xsl:call-template>
    ...
    <xsl:template name="ae-contents">
        ...
        <div class="ae_left_container_100pc">
            <div id="ae_header"><img src="{$context-path}/assets/images/ae_header.gif" alt="ArrayExpress"/></div>
            <xsl:choose>
                <xsl:when test="$vTotal&gt;0">
                    <div id="ae_results_header">
                        ...
                        <span id="ae_print_controls" class="noprint"><a href="javascript:window.print()"><img src="{$context-path}/assets/images/silk_print.gif" width="16" height="16" alt="Print"/>Print this window</a>.</span>
                    </div>
    ...
    <xsl:template match="experiment">
        ...
        <tr>
            <td class="col_accession">
                <div>
                    ...
                    <xsl:if test="not(user/@id = '1')">
                        <img src="{$context-path}/assets/images/silk_lock.gif" width="8" height="9"/>
                    </xsl:if>
                </div>
            </td>
            <td class="col_name">
            ...
            <td class="col_atlas">
                <div>
                    <xsl:choose>
                        <xsl:when test="@loadedinatlas"><a href="${interface.application.link.atlas.exp_query.url}{accession}"><img src="{$context-path}/assets/images/basic_tick.gif" width="16" height="16" alt="*"/></a></xsl:when>
                        <xsl:otherwise><img src="{$context-path}/assets/images/silk_data_unavail.gif" width="16" height="16" alt="-"/></xsl:otherwise>
                    </xsl:choose>
                </div>
            </td>
        </tr>
    ...
    <xsl:template name="data-files-main">
        ...
        <xsl:choose>
            <xsl:when test="0 != ae:getMappedValue($pKind, $pAccession)"><img src="{$context-path}/assets/images/basic_tick.gif" width="16" height="16" alt="*"/></xsl:when>
            <xsl:otherwise><img src="{$context-path}/assets/images/silk_data_unavail.gif" width="16" height="16" alt="-"/></xsl:otherwise>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="add-sort">
        ...
        <xsl:if test="$pKind = $vSortBy">
            <xsl:choose>
                <xsl:when test="'ascending' = $vSortOrder"><img src="{$context-path}/assets/images/mini_arrow_up.gif" width="12" height="16" alt="^"/></xsl:when>
                <xsl:otherwise><img src="{$context-path}/assets/images/mini_arrow_down.gif" width="12" height="16" alt="v"/></xsl:otherwise>
            </xsl:choose>
        </xsl:if>
    </xsl:template>
