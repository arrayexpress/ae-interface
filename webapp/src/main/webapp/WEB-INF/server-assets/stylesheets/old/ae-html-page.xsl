...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="page-header">
        ...
        <head>
            ...
            <link rel="stylesheet" href="{$context-path}/assets/stylesheets/ae_html_page_20.css" type="text/css"/>
    ...
    <xsl:template name="page-body">
        <body class="${interface.application.body.class}">
            ...
            <div id="ae_contents" class="ae_contents_frame ae_assign_font"><div id="ae_contents_container"><xsl:call-template name="ae-content"/></div></div>
            <noscript>
    ...
    <xsl:template name="page-body-plain">
        <body>
            <div id="ae_contents" class="ae_assign_font"><xsl:call-template name="ae-content"/></div>
        </body>
    </xsl:template>
    ...
    <xsl:template name="block-access-restricted">
        <xsl:call-template name="block-warning">
            ...
            <xsl:with-param name="pMessage">Sorry, the access to the resource you are requesting is restricted. You may wish to go <a href="javascript:history.back()" title="Click to go to the page you just left">back</a>, or to <a href="{$context-path}" title="ArrayExpress Home">ArrayExpress Home</a>.</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
    ...
    <xsl:template name="block-not-found">
        <xsl:call-template name="block-warning">
            ...
            <xsl:with-param name="pMessage">The resource you are requesting is not found. You may wish to go <a href="javascript:history.back()" title="Click to go to the page you just left">back</a>, or to <a href="{$context-path}" title="ArrayExpress Home">ArrayExpress Home</a>.</xsl:with-param>
        </xsl:call-template>
    </xsl:template>
