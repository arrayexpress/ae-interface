...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                        ...
                        <td class="value">
                            <div>
                                <xsl:for-each select="$vDates">
                                    ...
                                    <xsl:if test="(fn:name() = 'releasedate') and $pShowChangeReleaseDate"> (<a href="/fg/acext?acc={$vAccession}">change release date</a>&#160;<span class="new">new!</span>)</xsl:if>
                                </xsl:for-each>
                            </div>
                        </td>
                    </xsl:otherwise>
    ...
    <xsl:template name="exp-data-files">
        ...
        <xsl:variable name="vFiles" select="$pFiles/file[(@kind = 'raw' or @kind = 'processed' or (@kind='' and contains(@name,'.additional.'))) and @hidden != 'true']"/>
        <xsl:if test="$vFiles">
