...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                extension-element-prefixes="ae search html xs fn"
                exclude-result-prefixes="ae search html xs fn"
                version="2.0">
    ...
    <xsl:template match="/">
        <xsl:call-template name="ae-page-fluid">
            <xsl:with-param name="pTitle">Admin &lt; ArrayExpress &lt; EMBL-EBI</xsl:with-param>
            <xsl:with-param name="pExtraCSS"/>
            <xsl:with-param name="pIsSearchVisible" select="fn:true()"/>
            <xsl:with-param name="pBreadcrumbTrail">Admin</xsl:with-param>
            <xsl:with-param name="pExtraJS"/>
        </xsl:call-template>
    </xsl:template>

    <xsl:template name="ae-content">
